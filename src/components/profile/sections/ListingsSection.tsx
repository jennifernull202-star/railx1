/**
 * THE RAIL EXCHANGE™ — Listings Section
 * 
 * Displays seller listings on profile.
 * Uses marketplace empty state pattern if no listings.
 * NO mock data.
 */
...
import type { ListingsSectionProps } from '../types';

export function ListingsSection({ listings, maxItems = 6 }: ListingsSectionProps) {
  const displayedListings = listings?.slice(0, maxItems) || [];
  const hasListings = displayedListings.length > 0;
  const hasMore = (listings?.length || 0) > maxItems;
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {hasListings && (
          <span className="text-sm text-gray-500">
            {listings?.length} item{(listings?.length || 0) !== 1 ? 's' : ''}
          </span>
        )}
      </div>
