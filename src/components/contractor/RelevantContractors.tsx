...
export function RelevantContractors({
  ...
  // Get tier badge
  // S-5.1: Use 'Sponsored' for paid visibility tiers
  const getTierBadge = (tier: string) => {
    switch (tier) {
      case 'priority':
        return (
          <span 
            className="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-yellow-500 to-amber-500 text-white text-[10px] font-semibold rounded-full"
      ...
      case 'featured':
        return (
          <span 
            className="inline-flex items-center gap-1 px-2 py-0.5 bg-rail-orange text-white text-[10px] font-semibold rounded-full"
            title="This placement reflects paid visibility options, not contractor quality or endorsement."
          >
            Sponsored
          </span>
    ...
    <div className={`bg-gradient-to-br from-navy-50 to-navy-100/50 rounded-2xl border border-navy-200/50 p-6 ${className}`}>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-text-secondary">Contractors available for this equipment</p>
        </div>
      </div>
